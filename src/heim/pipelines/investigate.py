...
async def run_investigation(
    ...
    *,
    ...
    """Returns a result summary dict, or None if declined/timed out."""
    cfg = rt.config
    ...
    footer = f"{result.input_tokens:,} in · {result.output_tokens:,} out tokens · {len(result.steps)} commands"
...
async def dispatch_all(rt: Runtime, items: list[dict], *, concurrent: bool) -> None:
    reqs = [request_from_dispatch(i, rt) for i in items]
    if concurrent:
        for r in reqs:
            asyncio.create_task(run_investigation(rt, r))
    else:
        for r in reqs:
            await run_investigation(rt, r)
